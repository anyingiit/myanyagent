...
<!-- translation-of: README.md sha256:e6638349a64179ec -->
...
- Git
- Node.js 18 或更高版本——`install.sh` 与 `bin/myanyagent-bootstrap.sh` 在检测不到时都会拒绝运行
...
会报告机器级配置、私钥、以及当前仓库的 git 配置是否就绪；只要有一项没就绪，就会给出确切的下一步命令——`myanyagent-bootstrap` 用于配置某个仓库，或者在工具本身尚未安装时提示 `sh install.sh`。仓库一旦完成 bootstrap，后续通过 HTTPS 的 `git push` 和 `git fetch` 就会自动用安装令牌完成认证；`myanyagent-upstream` 以及其余的按仓库配置细节见 `docs/reference.md`。
